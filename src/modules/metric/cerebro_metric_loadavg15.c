/*****************************************************************************\
 *  $Id: cerebro_metric_loadavg15.c,v 1.1 2006-08-25 16:11:25 chu11 Exp $
 *****************************************************************************
...
#define LOADAVG15_FILE                "/proc/loadavg"
#define LOADAVG15_BUFLEN              4096
                                 ...
                                 void **metric_value)
{
  ...
  if ((fd = open(LOADAVG15_FILE, O_RDONLY, 0)) < 0)
    {
      CEREBRO_DBG(("open: %s", strerror(errno)));
      goto cleanup;
    }

  memset(buf, '\0', LOADAVG15_BUFLEN);
  if ((len = read(fd, buf, LOADAVG15_BUFLEN)) < 0)
    {
  ...
  if (sscanf(buf, "%f %f %f", &loadavg1, &loadavg5, &loadavg15) != 3)
    {
      CEREBRO_DBG(("loadavg15 file parse error"));
      goto cleanup;
    }
